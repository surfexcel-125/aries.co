...
// This module is loaded by /src/core/module-loader.js via a default FUNCTION export.

import { apply as applyUiKit } from '../shared/ui-kit.js';

export default function dashboardModule(container, project, saved, api) {
  // ---------------------------------------------------------------------------
  // INITIAL SETUP
  // ---------------------------------------------------------------------------
  ...
  // Dummy data – later you can feed from backend/project
  const KPI_SETS = {
    my: {
      today: [
  ...
  const rootEl       = container.querySelector('.mw-dashboard');
  const kpisEl       = container.querySelector('[data-db-widget="kpis"]');
  const todayEl      = container.querySelector('[data-db-widget="today"]');
  const activityEl   = container.querySelector('[data-db-widget="activity"]');
  const highlightsEl = container.querySelector('[data-db-widget="highlights"]');

  const viewButtons      = Array.from(container.querySelectorAll('[data-db-view]'));
  const timeframeButtons = Array.from(container.querySelectorAll('[data-db-timeframe]'));
  const focusToggleEl    = container.querySelector('[data-db-focus-toggle]');
  const searchInputEl    = container.querySelector('[data-db-search]');
  ...
  function renderActivity() {
    ...
    // re-bind tab buttons after re-render
    const activityTabButtons = Array.from(activityEl.querySelectorAll('[data-db-activity-tab]'));
